...
> 基于对 `D:\Codes\baas\appwrite` 和 `D:\Codes\qiulin\fleetwork` 的二次 review，列出当前 P0 设计与两者的对齐点、偏差点、以及需要进一步确认/补充的事项。
...
| 配置 | protobuf config + `FLEET_` 环境变量 | fleetwork 标准 | 沿用 |
...
1. 明确 P0 cookie 格式：`fleet_session_{project_id}` 存 signed session id；服务端用 `sessions` 表验证。
